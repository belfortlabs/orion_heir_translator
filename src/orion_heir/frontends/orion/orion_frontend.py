...
def fix_encode_operations(module, type_builder):
        ...
        if isinstance(op, (AddPlainOp, SubPlainOp, MulPlainOp)):
            ...
            if isinstance(plaintext_operand.owner, RLWEEncodeOp):
                ...
                required_scale = (
                    ...
                    else getattr(type_builder.scheme_params, "log_scale", 40)
                )
    ...
    for fix in fixes_needed:
        ...
        new_pt_type = type_builder.create_plaintext_type_with_scale(
            log_scale=required_scale, match_ciphertext_type=fix["ciphertext_type"]
        )
...
class OrionFrontend(FrontendInterface):
    ...
    def extract_operations(self, source: Any) -> List[FHEOperation]:
        ...
        all_layers = list(source.named_modules())
        ...
        for name, layer in all_layers:
            ...
            if self._should_extract_layer(layer):
                ...
                if layer_type in ("Linear", "Embedding"):
                    ...
                    print(
                        f"    ℹ️  Flatten {name} is plaintext operation (handled by Linear diagonals)"
                    )
    ...
    def _should_extract_layer(self, layer: Any) -> bool:
        ...
        # Embedding (HE-LRM) is a Linear subclass with bias=False; treat it the
        # same so its weight matrix becomes a linear_transform.
        if layer_type in ("Linear", "Embedding"):
    ...
    def _generate_output_rotations(
        self, layer: Any, layer_name: str, base_result_var: str, level: int
    ) -> List[FHEOperation]:
        """
    ...
    def _get_linear_operations(self, layer: Any, layer_name: str) -> List[FHEOperation]:
        ...
        rotation_ops = self._generate_output_rotations(
            layer, layer_name, f"{layer_name}_linear", level
        )
        ...
        if rotation_ops:
            ...
            for op in reversed(rotation_ops):
                if op.op_type == "add" and "acc" in (op.result_var or ""):
    ...
    def extract_orion_diagonals(self, layer: Any) -> Optional[torch.Tensor]:
        ...
        if not hasattr(layer, "diagonals") or not layer.diagonals:
            print("      ❌ No diagonals attribute or empty diagonals")
            return None

        print("      🔍 Orion diagonals structure:")
        ...
        if not diagonal_list:
            print("      ❌ No valid diagonal data found")
            return None
    ...
    def _get_conv_operations(
        ...
        operations.append(
            FHEOperation(
                ...
                method_name="linear_transform",
                args=[layer],
                kwargs={"matrix_type": "toeplitz_diagonal", "convolution": True},
        ...
        rotation_ops = self._generate_output_rotations(
            layer, layer_name, f"{layer_name}_conv", level
        )
        operations.extend(rotation_ops)
        ...
        if rotation_ops:
            final_result = None
            for op in reversed(rotation_ops):
                if op.op_type == "add" and "acc" in op.result_var:  # type: ignore
                    final_result = op.result_var
                    break
            if not final_result:
    ...
    def _get_batchnorm_operations(self, layer: Any, layer_name: str) -> List[FHEOperation]:
        ...
        if weight is not None and running_var is not None:
            ...
            fused_bias = (
                bias - running_mean * fused_weight
                if bias is not None and running_mean is not None
                else None
            )
        ...
        if hasattr(layer, "running_mean") and hasattr(layer, "running_var"):
            ...
            operations.append(
                FHEOperation(
                    ...
                    level=level,
                    metadata={
                        "operation": "batchnorm_scale_encode",
                        "layer": layer_name,
            ...
            operations.append(
                FHEOperation(
                    ...
                    level=level,
                    metadata={
                        "operation": "batchnorm_bias_encode",
                        "layer": layer_name,
                        "layer_type": "BatchNorm2d",
                    },
                )
            ...
            operations.append(
                FHEOperation(
                    ...
                    level=level,
                    metadata={
                        "operation": "batchnorm_bias_addition",
                        "layer": layer_name,
                        "layer_type": "BatchNorm2d",
                    },
                )
    ...
    def _get_chebyshev_operations(self, layer: Any, layer_name: str) -> List[FHEOperation]:
        ...
        print(
            f"       - All attributes: {[attr for attr in dir(layer) if not attr.startswith('_')]}"
        )
    ...
    def _get_activation_operations(self, layer: Any, layer_name: str) -> List[FHEOperation]:
        ...
        Orion's ``Activation`` / ``Quad`` layers expose either Chebyshev
        coefficients (with an approximation domain) or are pure ``x^2``
        operations. We dispatch accordingly:

        * ``Quad`` -> a single self-multiplication op (handled by
          ``_get_quad_operations``).
        * Anything else with ``coeffs`` -> a Chebyshev approximation op
          (handled by ``_get_chebyshev_operations``).
        * Otherwise -> no operations.
        """
        layer_type = layer.__class__.__name__

        if layer_type == "Quad":
            return self._get_quad_operations(layer, layer_name)
    ...
    def extract_scheme_parameters(
        self, source: Union[str, Path, Dict[str, Any], Any]
    ) -> SchemeParameters:
        """
